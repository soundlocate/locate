...
union Vector2D {
    ...
    auto norm() -> decltype(x * x + y * y) { return std::sqrt(x * x + y * y); }
};
...
namespace std {
    template <typename valueType>
    struct hash<Vector2D<valueType>>
        : public std::unary_function<const Vector2D<valueType> &, std::size_t> {
    ...
    template <typename valueType>
    struct hash<Vector4D<valueType>>
        : public std::unary_function<const Vector4D<valueType> &, std::size_t> {
