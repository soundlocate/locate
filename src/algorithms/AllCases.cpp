...
AllCases::AllCases(std::vector<Microfone> mics, u64 numMics, f64 accuracy)
    : numMics(numMics), accuracy(accuracy) {

	f64 dist;
	f64 smallestDist = sound::speedInAir / maxFreq;
	f64 ratio;
	u64 maxCases = 0;

	for(u64 i = 1; i < numMics; i++) {
		dist = (mics[0].pos - mics[i].pos).norm();
		ratio = dist / smallestDist;
...
v3 AllCases::locate(std::vector<Microfone> mics, FFTPacket::Sinus * p,
    ...
    while(d.norm() > accuracy) {
        ...
        for(u64 i = 0; i < numMics; i++) { r[i] = (mics[i].pos - pos).norm(); }

        // fill the matrix
        for(u64 i = 0; i < numMics - 1; i++) {
            matrix[3 * i + 0] = ((pos.x - mics[i + 1].pos.x) / r[i + 1]) -
                                ((pos.x - mics[0].pos.x) / r[0]);
            matrix[3 * i + 1] = ((pos.y - mics[i + 1].pos.y) / r[i + 1]) -
                                ((pos.y - mics[0].pos.y) / r[0]);
            matrix[3 * i + 2] = ((pos.z - mics[i + 1].pos.z) / r[i + 1]) -
                                ((pos.z - mics[0].pos.z) / r[0]);
        }
        ...
        // solve least squares
        LAPACKE_dgelsd(LAPACK_ROW_MAJOR, numMics - 1, 3, 1, matrix, 3, target,
                       1, singular, -1, &rank);
        ...
        nitr++;
        if(nitr > 50) {
            //				std::cerr << "did not converge for freq: "
            //<<
            // freq
            //<<
            // std::endl;

            // lets not save that one, it is totally wrong, lets also reset the
            // lastPosition to center
