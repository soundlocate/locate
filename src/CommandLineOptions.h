...
private:
    ...
    // super(pd, "soundLocate", "-d " + dataAlgorithms, "-a " + accuracy, "-c "
    // + maxClusterSize, "-k" + maxKeep, "-m " + meanWindow, logfile != null ?
    ...
    struct argp_option options[10] = {
        ...
        {"meanwindow", 'w', "COUNT", 0, "size of averaging windowx", 0},
        {"timekeep", 't', "DOUBLE", 0, "time to keep old positions", 0},
        {"logfile", 'l', "FILE", 0,
         "if set use set logfile to save located positions", 0},
        {"positionfile", 'p', "FILE", 0, "filename of micfile", 0},
        {"dissimilarity", 'f', "DOUBLE", 0,
         "which dissimilarity function to use [meanDist, meanDirection]", 0},
        {0, 0, 0, 0, 0, 0}};
    ...
    static error_t parse_opt(int key, char * arg, struct argp_state * state) {
        ...
        case 'f': {
            ...
            if(s == "meanDist") {
                ...
                std::cerr << "invalid dissimilarity function " << arg
                          << std::endl;
                argp_usage(state);
            }
