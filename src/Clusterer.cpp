...
Clusterer::Clusterer(Dissimilarity * d) : dissimilarity(d) {
    assert(d != nullptr);
}

std::vector<Position> Clusterer::cluster(PositionCache positions) {
    clusters.clear();
    pos.clear();
    ...
    do {
        ...
        for(u64 i = 0; i < pos.size(); i++) {
            ...
            if(mergeCanidateFound &&
               dissimilarity->shouldMerge(dsim, pos[i], pos[other])) {
    ...
    } while(hasMerged); /* try and merge clusters until no cluster gets merged
                           anymore */
