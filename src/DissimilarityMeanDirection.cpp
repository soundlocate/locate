...
DissimilarityMeanDirection::DissimilarityMeanDirection(
    std::vector<Microfone> mics, f64 maxClusterSize)
    : maxClusterSize(maxClusterSize) {
    // calculate center of mics
    v3 center;

    for(auto mic : mics) { center += mic.pos; }
...
f64 DissimilarityMeanDirection::dissimilarity(const Cluster & a,
                                              const Cluster & b) {
    f64 ret = 0;
    f64 tmp = 0;
    f64 min;
    ...
    posABuffer.clear();
    posBBuffer.clear();
    for(auto pos : a.objs) { posABuffer.push_back(pos.pos - micCenter); }
    for(auto pos : b.objs) { posBBuffer.push_back(pos.pos - micCenter); }
...
u64 DissimilarityMeanDirection::shouldMerge(const f64 &     dissimilarity,
                                            const Cluster & a,
