...
FFTStream::FFTStream(const char * ip, unsigned short port) {
	...
	handle = new std::thread([&] () {
			...
			while(true) {
				...
				while(server->receive(&buf[to_receive_size - toReceive], toReceive, receivedSize) != sf::Socket::NotReady) {
					toReceive -= receivedSize;

					// receive a whole packet
					while(toReceive > 0) {
						s = server->receive(&buf[to_receive_size - toReceive], toReceive, receivedSize);
